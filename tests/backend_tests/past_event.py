...
class TestPastEventBackend(BackendTest):
    ...
    def test_delete_past_course_cascade(self) -> None:
        ...
        with self.assertRaises(ValueError) as e:
            self.pastevent.delete_past_course(self.key, 1, cascade=("genesis_cases",))
        self.assertIn("participants", e.exception.args[1].get('block'))
        with self.assertRaises(ValueError) as e:
            self.pastevent.delete_past_course(self.key, 1, cascade=("participants",))
    ...
    def test_delete_past_event_cascade(self) -> None:
        ...
        with self.assertRaises(ValueError) as e:
            self.pastevent.delete_past_event(
                self.key, 1, cascade=("courses", "log", "genesis_cases")
            )
        self.assertIn("participants", e.exception.args[1].get('block'))
        with self.assertRaises(ValueError) as e:
            self.pastevent.delete_past_event(
                self.key, 1, cascade=("courses", "participants", "genesis_cases")
            )
        self.assertIn("log", e.exception.args[1].get('block'))
        with self.assertRaises(ValueError) as e:
            self.pastevent.delete_past_event(
    ...
    def test_entity_participant(self) -> None:
        personas = self.core.get_core_users(self.key, [2, 3, 4, 5, 6, 100])
        ...
        expectation = {
            vtypes.ID(2): models.PastEventParticipant(
                id=vtypes.ID(1),
                persona_id=vtypes.ID(2),
                pevent_id=vtypes.ID(1),
        ...
        expectation[vtypes.ID(2)].course_assignments = [
            models.PastCourseAssignment(
                id=vtypes.ID(1),
                persona_id=vtypes.ID(2),
                participant_id=vtypes.ID(1),
                pcourse_id=vtypes.ID(1),
                instructor_status=const.PastInstructorKind.kl,
                pcourse=pcourses[1],
            )
        ]
        expectation[vtypes.ID(4)].course_assignments = [
            models.PastCourseAssignment(
                id=vtypes.ID(2),
                persona_id=vtypes.ID(4),
                participant_id=vtypes.ID(3),
                pcourse_id=vtypes.ID(2),
        ...
        expectation[vtypes.ID(5)].course_assignments = [
            models.PastCourseAssignment(
                id=vtypes.ID(3),
                persona_id=vtypes.ID(5),
                participant_id=vtypes.ID(4),
                pcourse_id=vtypes.ID(2),
                instructor_status=const.PastInstructorKind.none,
                pcourse=pcourses[2],
            )
        ]
        expectation[vtypes.ID(6)].course_assignments = [
            models.PastCourseAssignment(
                id=vtypes.ID(4),
                persona_id=vtypes.ID(6),
                participant_id=vtypes.ID(5),
                pcourse_id=vtypes.ID(2),
                instructor_status=const.PastInstructorKind.none,
                pcourse=pcourses[2],
            )
        ]
        expectation[vtypes.ID(100)].course_assignments = [
            models.PastCourseAssignment(
                id=vtypes.ID(5),
                persona_id=vtypes.ID(100),
                participant_id=vtypes.ID(6),
                pcourse_id=vtypes.ID(2),
                instructor_status=const.PastInstructorKind.none,
                pcourse=pcourses[2],
            )
        ...
        self.assertEqual(
            (6, expectation),
            self.pastevent.list_event_participants(self.key, pevent_id=1),
        )
        ...
        self.assertEqual(
            0, self.pastevent.remove_participant(self.key, pevent_id=1, persona_id=1)
        )
        self.assertEqual(
            0,
            self.pastevent.remove_course_assignment(
                self.key, pcourse_id=1, persona_id=1
            ),
        )
        self.assertEqual(
            (6, expectation),
        ...
        self.assertEqual(
            1, self.pastevent.remove_participant(self.key, pevent_id=1, persona_id=5)
        )
        ...
        self.pastevent.set_participant(
            self.key, pevent_id=1, persona_id=5, orga_status=const.PastOrgaKind.al
        )
        ...
        expectation[vtypes.ID(5)].course_assignments.append(
            models.PastCourseAssignment(
                id=vtypes.ID(1001),
                persona_id=vtypes.ID(5),
                participant_id=vtypes.ID(1002),
                pcourse_id=vtypes.ID(2),
                pcourse=pcourses[vtypes.ID(2)],
                instructor_status=const.PastInstructorKind.none,
            )
        )
        self.assertEqual(
    ...
    def test_past_log(self) -> None:
        ...
        self.pastevent.set_past_event(
            self.key, new_id, {'title': "Alternate Universe Academy"}
        )
        data = {
            'pevent_id': 1,
            'nr': '0',
        ...
        self.pastevent.set_past_course(
            self.key, {'id': new_id, 'title': "New improved title"}
        )
    ...
    def test_archive(self) -> None:
        ...
        update = {
            ...
            'registration_hard_limit': datetime.datetime(
                2002, 10, 30, 0, 0, 0, tzinfo=datetime.UTC
            ),
            'parts': {
                1: {
        ...
        self.assertEqual(
            set(),
            set(self.pastevent.list_past_courses(self.key, pevent_data[2].id).values()),
        )
        ...
        self.assertEqual(
            expectation,
            set(self.pastevent.list_past_courses(self.key, pevent_data[1].id).values()),
        )
        _, participants = self.pastevent.list_event_participants(
            self.key, pevent_data[1].id
        )
        ...
        self.assertEqual(
            "Lustigsein für Fortgeschrittene",
            participants[7].course_assignments[0].pcourse.title,
        )
        ...
        self.assertEqual(
            expectation,
            set(self.pastevent.list_past_courses(self.key, pevent_data[0].id).values()),
        )
        _, participants = self.pastevent.list_event_participants(
            self.key, pevent_data[0].id
        )
        self.assertEqual(
            const.PastInstructorKind.kl,
            participants[5].course_assignments[0].instructor_status,
        )
        self.assertEqual(
            const.PastInstructorKind.none,
            participants[100].course_assignments[0].instructor_status,
        )
