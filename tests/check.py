...
def _prepare_test_environment(thread_id: int = 1) -> None:
    """Prepare environment variables for tests."""
    os.environ['CDEDB_TEST'] = "True"
    os.environ['CDEDB_TEST_DATABASE'] = f'cdb_test_{thread_id}'
    tmp_dir = pathlib.Path(f'/tmp/cdedb-test-{thread_id}')
    os.environ['CDEDB_TEST_TMP_DIR'] = str(tmp_dir)
    # TODO: implement lock mechanism to prevent parallel runs with the same id


def _prepare_check() -> None:
    # TODO implement this directly, don't use Makefile
...
def check_xss(thread_id: int = 1, manual_preparation: bool = False) -> int:
    ...
    if not manual_preparation:
        _prepare_test_environment(thread_id=thread_id)
        _prepare_check()
...
if __name__ == '__main__':
    ...
    thread_options.add_argument('--thread_id', type=int, choices=(1, 2, 3, 4),
                                default=1, metavar="INT",
                                help="ID of thread to use for run (this is useful if"
                                     " you manually start multiple test suite runs in"
                                     " parallel)")
    ...
    if args.xss_check:
        sys.exit(
            check_xss(manual_preparation=args.manual_preparation)
        )

    sys.exit(
        run_testsuite(args.testpatterns, thread_id=args.thread_id,
