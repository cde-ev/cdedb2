...
# dPROD: SECRETS_CONFIGPATH = pathlib.Path("/etc/cdedb/secrets.py")
...
# dPROD: LDAP_HOST = "ldap.cde-ev.de"
...
# TODO: Install these under /etc/ssl/ldap for the auto-build too.
...
# dPROD: LDAP_PEM_PATH = pathlib.Path("/var/local/letsencrypt/certs/ldap.cde-ev.de.pem")
# dPROD: LDAP_KEY_PATH = pathlib.Path("/etc/letsencrypt/domainkeys/ldap.cde-ev.de.key")

if _pathlib.Path("/CONTAINER").is_file():
    ...
    LDAP_HOST = "ldap"
    LDAP_PEM_PATH = _pathlib.Path("/etc/ssl/ldap/ldap.pem")
    LDAP_KEY_PATH = _pathlib.Path("/etc/ssl/ldap/ldap.key")
...
if _pathlib.Path("/CONTAINER").is_file():
    # In docker all log messages are written to stdout, so reduce the noise.
    LOG_LEVEL = _logging.INFO
elif _pathlib.Path("/PRODUCTIONVM").is_file():
    # Avid logging sensitive data in production.
    LOG_LEVEL = _logging.INFO
...
# maximum length of personas presented via select persona API for any other user
...
# dPROD: MAILMAN_HOST = "10.10.0.2:8001"
