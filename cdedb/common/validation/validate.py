...
def _event_track_group(
    ...
    if creation:
        if "track_ids" not in val or not val["track_ids"]:
            ...
            if len({(event.tracks[track_id].num_choices, event.tracks[track_id].min_choices)
                    for track_id in val["track_ids"]}) != 1:
                errs.append(ValueError("track_ids", n_("Incompatible tracks.")))
