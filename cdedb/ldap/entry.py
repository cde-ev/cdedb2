...
class CdEDBBaseLDAPEntry(
    ...
    async def search(
        ...
        attributes: Optional[AttributeDescriptionList] = (),
        ...
        if scope == pureldap.LDAP_SCOPE_wholeSubtree:
            ...
            entries = await self.children(bound_dn, filterObject=filterObject, attributes=attributes)
    ...
    @abc.abstractmethod
    async def children(
        self, bound_dn: Optional[BoundDn] = None, *, filterObject: Optional[pureldap.LDAPFilter] = None, attributes: Optional[AttributeDescriptionList] = ()
    ) -> LDAPEntries:
    ...
    async def subtree(
        ...
        result = [self]
        children = await self.children(bound_dn=bound_dn, filterObject=filterObject, attributes=attributes)
        subtrees = await asyncio.gather(
            *[child.subtree(bound_dn=bound_dn, filterObject=filterObject, attributes=attributes) for child in children])
...
class CdEPreLeafEntry(CdEDBStaticEntry, metaclass=abc.ABCMeta):
    ...
    async def children_lister(
            self, bound_dn: Optional[BoundDn] = None, *, filterObject: Optional[pureldap.LDAPFilter] = None
    ...
    @abc.abstractmethod
    async def children_getter(self, dns: list[DistinguishedName], *, attributes: Optional[AttributeDescriptionList] = ()) -> LDAPObjectMap:
    ...
    async def children(self, bound_dn: Optional[BoundDn] = None, *, filterObject: Optional[pureldap.LDAPFilter] = None, attributes: Optional[AttributeDescriptionList] = ()) -> LDAPEntries:
